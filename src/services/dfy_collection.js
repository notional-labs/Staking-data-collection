...
const constructTokenContract = require('../configs/blockchain/contract/token_contract');
...
async function processTransferEvent(blocks, web3) {
    ...
    const contract = new web3.eth.Contract(abi, process.env.DFY_CONTRACT_ADDRESS, {
    ...
    for (const wallet of accountSet) {
        ...
        let promise = new Promise(async function() {
            while(true) {
                try {
                    trail ++;
                    let balance = new BigNumber(await contract.methods.balanceOf(wallet).call({}));
